...
def assert_interview_access(db: Session, *, interview: Interview, user: User) -> None:
    ...
    if user.role == UserRole.hr:
        ...
        # Mock interviews: HR can view if candidate has applied to HR's jobs
        candidate_job_stmt = (
            select(func.count(Application.id))
            .join(Job, Job.id == Application.job_id)
            .where(Application.candidate_user_id == interview.candidate_user_id, Job.hr_user_id == user.id)
        )
        if int(db.scalar(candidate_job_stmt) or 0) > 0:
            return
...
def complete_interview(
    ...
    *,
    ...
    while len(normalized_answers) < len(questions):
        normalized_answers.append("No answer provided")
        logger.warning("padding_missing_answer", interview_id=interview.id, total_questions=len(questions), provided_answers=len(answers))
    ...
    if not transcript or not transcript.strip():
        ...
        transcript_parts = []
        for q, a in zip(questions, normalized_answers):
            transcript_parts.append(f"AI: {q.question}")
            transcript_parts.append(f"Candidate: {a}")
        transcript = "\n".join(transcript_parts)

    logger.info("scoring_interview", interview_id=interview.id, questions=len(questions), answers=len(normalized_answers), transcript_length=len(transcript))
    ...
    try:
        score = score_interview(questions=questions, answers=normalized_answers, transcript=transcript)
