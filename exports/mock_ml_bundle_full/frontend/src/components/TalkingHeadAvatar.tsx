...
export const TalkingHeadAvatar = forwardRef<TalkingHeadHandle, TalkingHeadAvatarProps>(({ onReady }, ref) => {
  ...
  const speakWithServerTts = async (text: string) => {
    ...
    try {
      ...
      if (headRef.current?.speakAudio && audioBuffer) {
        console.log("Playing audio with avatar lip-sync");
        
        await new Promise<void>((resolve, reject) => {
          const markerTime = Math.max(durationMs, wtimes[wtimes.length - 1] ?? 0);
          const marker = () => {
            console.log("Speech marker reached");
            resolve();
          };
      ...
      console.log("Playing audio without lip-sync");
  ...
  useImperativeHandle(ref, () => ({
    speak: async (text: string) => {
      ...
      try {
        ...
        if (ttsRef.current && headRef.current) {
          ...
          await audioPromise;
          console.log("Local TTS successful");
          return;
        }
    ...
    enableAudio: async () => {
      console.log("Enabling audio contexts");
      ...
      if (audioCtx && audioCtx.state === "suspended") {
        try {
          ...
          console.log("Local TTS audio context resumed");
        } catch (err) {
      ...
      if (decodeCtxRef.current && decodeCtxRef.current.state === "suspended") {
        try {
          ...
          console.log("Decode audio context resumed");
        } catch (err) {
      ...
      if ("speechSynthesis" in window) {
        try {
          ...
          console.log("Browser speech synthesis resumed");
        } catch (err) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        ttsRef.current.onmessage = (message: any) => {
          if (message?.type === "audio" && headRef.current) {
            console.log("Received audio from HeadTTS");
            try {
        ...
        try {
          ...
          console.log("HeadTTS initialized successfully");
        } catch (err) {
    ...
    return () => {
      console.log("Unmounting avatar component");
      isMounted = false;
