...
from app.schemas.job import JobBrowseResponse, JobDetailResponse
from app.services.application_service import apply_to_job, serialize_application
from app.services.external_job_service import list_external_jobs_for_candidate
from app.services.job_service import get_job, get_job_detail_for_candidate, list_jobs_for_candidate
...
@router.get("/external", response_model=JobBrowseResponse)
async def browse_external_jobs(
    resume_id: int | None = Query(default=None, gt=0),
    page: int = Query(default=1, ge=1),
    page_size: int = Query(default=12, ge=1, le=50),
    keyword: str | None = Query(default=None),
    location: str | None = Query(default=None),
    job_category_code: str | None = Query(default=None),
    current_user: User = Depends(get_current_candidate),
    db: Session = Depends(get_db),
) -> JobBrowseResponse:
    items, meta = list_external_jobs_for_candidate(
        db,
        candidate=current_user,
        resume_id=resume_id,
        page=page,
...
async def job_detail(
    ...
    item = get_job_detail_for_candidate(
        db,
        candidate=current_user,
        job_id=job_id,
        resume_id=resume_id,
    )
    return JobDetailResponse(**item)
