...
router = APIRouter(prefix="/api/candidate/notifications", tags=["candidate-notifications"])
...
async def unread_count(current_user: User = Depends(get_current_candidate), db: Session = Depends(get_db)) -> NotificationUnreadCountResponse:
