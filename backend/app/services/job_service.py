...
def list_jobs_for_candidate(
    ...
    *,
    ...
    stmt = (
        select(Job)
        .options(selectinload(Job.company), selectinload(Job.hr_user))
...
def get_job_detail_for_candidate(
    db: Session,
    *,
    candidate: User,
    job_id: int,
    ...
    application_map = _application_map(db, candidate_user_id=candidate.id, job_ids=[job.id])
    application = application_map.get(job.id)
...
def applicants_for_job(
    ...
    *,
    ...
    for application, resume, ai_interview, candidate_user in rows:
        report_ready = bool(ai_interview and ai_interview.status and ai_interview.status.value == "completed")
        payload.append(
            {
                "application": {
                    "id": application.id,
                ...
                "ai_interview": {
                    "interview_id": ai_interview.id if ai_interview else None,
                    ...
                    "report_pdf_url": f"/api/hr/interviews/{ai_interview.id}/report.pdf" if ai_interview else None,
                    "completed_at": ai_interview.completed_at.isoformat() if ai_interview and ai_interview.completed_at else None,
                },
            }
        )
