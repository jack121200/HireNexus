...
def _is_skip_or_no_answer(answer: str) -> bool:
    ...
    *,
    ...
    try:
        ...
        for idx, item in enumerate(per_question_raw):
            ...
            answer_index = idx_map.get(question.id, idx)
...
def score_interview(*, questions: list[QuestionItem], answers: list[str], transcript: str) -> ScoreResult:
    ...
    for question, answer in zip(questions, padded_answers, strict=False):
        if _is_skip_or_no_answer(answer):
            scores.append(0.0)
            per_question.append({
                "id": question.id,
                "question": question.question,
                ...
                "score": 0.0,
                "components": {"semantic": 0.0, "coverage": 0.0, "coherence": 0.0},
                "feedback": "Candidate skipped or could not answer this question.",
                "answer_tag": "skip_or_no_answer",
            })
            continue
