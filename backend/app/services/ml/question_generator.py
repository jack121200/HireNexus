...
def detect_skip_intent(answer: str) -> bool:
    """Detect when candidate explicitly wants to skip or cannot answer."""
    text = (answer or "").strip().lower()
    if not text:
        return True
    return any(pattern in text for pattern in _SKIP_INTENT_PATTERNS)


def _cap_to_two_sentences(text: str) -> str:
...
def _likely_complete_answer(answer: str) -> bool:
    ...
    if word_count < 16:
        return False
    markers = ("because", "for example", "for instance", "trade-off", "tradeoff", "approach", "implemented")
    lowered = text.lower()
...
def generate_dynamic_question(
    *,
    ...
    if not allow_template_fallback:
        focus_skill = ""
        if context.job_required_skills:
            focus_skill = context.job_required_skills[index % len(context.job_required_skills)]
        elif context.resume_skills:
            focus_skill = context.resume_skills[index % len(context.resume_skills)]
        role = context.role or "this role"
        skill_hint = f" around {focus_skill}" if focus_skill else ""
        return QuestionItem(
            id=f"q{index + 1}",
            question=f"Walk me through a real-world challenge you solved as a {role}{skill_hint}, including trade-offs.",
            difficulty="medium" if context.years_experience < 5 else "hard",
            category="scenario",
            rubric_points=("Context and challenge", "Technical decisions and trade-offs", "Outcome and learnings"),
        )
...
def generate_interview_reply(
    *,
    ...
    settings = get_settings()
    provider = settings.qgen_provider.strip().lower()
    skip_intent = detect_skip_intent(answer)

    if skip_intent:
        return {
            "reply": "No problem. Give me a quick high-level approach you would try first.",
            "followup": "What would be your first step if you had to start solving this right now?",
            "move_on": False,
            "skip_intent": True,
            "fallback_reason": "skip_intent_detected",
        }
    ...
    if provider == LLMProvider.GEMINI.value:
        ...
        if isinstance(reply_payload, dict):
            reply = _cap_to_two_sentences(str(reply_payload.get("reply", "")).strip())
            ...
            if not move_on:
                move_on = _likely_complete_answer(answer)
            
            # Validate response
            if not reply and followup_text:
                return {
                    "reply": _cap_to_two_sentences(followup_text),
                    "followup": None,
                    "move_on": False,
                    "skip_intent": False,
                    "fallback_reason": "gemini_followup_as_reply",
                }
            if reply:
                logger.info("generate_reply_gemini_success", move_on=move_on)
                return {
                    "reply": reply,
                    "followup": _cap_to_two_sentences(followup_text) if followup_text else None,
                    "move_on": move_on,
                    "skip_intent": False,
    ...
    if provider in {LLMProvider.GROQ.value, "grok", LLMProvider.LOCAL.value, "local_llm"}:
        ...
        if followup:
            ...
            return {
                "reply": _cap_to_two_sentences("Good point. Let me ask one quick clarification."),
                "followup": _cap_to_two_sentences(followup),
                "move_on": False,
                "skip_intent": False,
                "fallback_reason": "llm_followup_only",
            }
    ...
    move_on = _likely_complete_answer(answer)
    if move_on:
        reply = "Thanks, that's clear. Let's move to the next question."
        logger.info("generate_reply_template_move_on")
        return {
            "reply": _cap_to_two_sentences(reply),
            "followup": None,
            "move_on": True,
            "skip_intent": False,
            "fallback_reason": "template_move_on",
        }

    logger.info("generate_reply_template_fallback")
    return {
        "reply": _cap_to_two_sentences("Thanks. I need one more detail before we move on."),
        "followup": _cap_to_two_sentences(followup),
        "move_on": False,
        "skip_intent": False,
        "fallback_reason": "template_followup",
    }
